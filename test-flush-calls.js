  ...
  try {
    ...
    const flushTest = await page.evaluate(() => {
      ...
      gl.drawElements = function(mode, count, type, offset) {
        ...
        console.log(`Draw call ${drawCalls}: count=${count}, VAO=${currentVAO ? 'bound' : 'unbound'}, program=${currentProgram ? 'bound' : 'unbound'}`);
    ...
    if (flushTest.pipelineWorking) {
      ...
      if (flushTest.lastVertexData) {
        console.log('📦 Last vertex data sample:', flushTest.lastVertexData.slice(0, 8));
      }
    } else {
